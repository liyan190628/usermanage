<template>
  <div class="bg-white p-all">
    <el-row :gutter="20">
      <el-col :span="16">
        <!-- pump model -->
        <el-row type="flex" class="mb-20 pd" style="background: #f0f0f0;padding: 12px;" justify="center">
          <el-col :span="6">
            <h3>pump model</h3>
          </el-col>
        </el-row>
        ...
        <el-row class="border bg-white mb-20 pr-20">
          <el-col class="text-center pb-20 pt-10" :span="21">System Overview</el-col>
            ...
            <el-form :model="productAdvantage" ref="productAdvantage">
              <el-form-item v-for="(domain, index) in productAdvantage.domains" :key="domain.key" :prop="'domains.' + index + '.value'">
                <el-input v-model="domain.value">
                  <i v-if="index" class="el-icon-minus el-input__icon" slot="suffix" @click="productAdvantageReduce(index)"></i>
                  <i class="el-icon-plus el-input__icon" slot="suffix" @click="productAdvantageAdd"></i>
                </el-input>
              </el-form-item>
            </el-form>
          </el-row>
              ...
              <el-row class="py pb-10">
                <el-form :model="technicalData" ref="technicalData">
                  <el-form-item v-for="(domain, index) in technicalData.domains" :key="domain.key" :prop="'domains.' + index + '.value'">
                    <el-input v-model="domain.value">
                      <i v-if="index" class="el-icon-minus el-input__icon" slot="suffix" @click="technicalDataReduce(index)"></i>
                      <i class="el-icon-plus el-input__icon" slot="suffix" @click="technicalDataAdd"></i>
                    </el-input>
                  </el-form-item>
                </el-form>
              </el-row>
        ...
        <el-row class="authentication border bg-white mb-20 pt-10">
          ...
          <el-row class="pd py">
            <el-checkbox-group v-model="checkList">
              <el-checkbox v-for="item in standards" :key="item.key" :label="item.label"></el-checkbox>
            </el-checkbox-group>
          </el-row>
        </el-row>
          ...
          <el-row class="py pb-10">
            <el-form :model="explain" ref="explain">
              <el-form-item v-for="(domain, index) in explain.domains" :key="domain.key" :prop="'domains.' + index + '.value'">
                <el-input v-model="domain.value">
                  <i v-if="index" class="el-icon-minus el-input__icon" slot="suffix" @click="explainReduce(index)"></i>
                  <i class="el-icon-plus el-input__icon" slot="suffix" @click="explainAdd"></i>
                </el-input>
              </el-form-item>
            </el-form>
          </el-row>
      ...
      <!-- 上传图片 -->
      <el-col :span="8">
        <el-upload class="avatar-uploader" :action="st" :show-file-list="false" :http-request="uploadImg">
    ...
    export default {
        data() {
            return {
                ...
                imageUrl: '',
                // 'http://172.105.200.91:8080/pumpms/upload/picture/20190109133017388.jpg',
                st: '',
                types: [], // motor类型
                standards: [] // 多选框
            }
        ...
        methods: {
            uploadImg(item) {
                ...
                this.$axios
                    ...
                    .then(res => {
                        ...
                        // this.imageUrl =
                        //   'http://172.105.200.91:8080/pumpms/upload/picture/' + arr[5]
            ...
            confirm() {
                ...
                this.$refs['systemItems'].validate(valid => {
                    if (valid) {
                        this.$axios.post('/pumpms/pump/add', vm).then(res => {
                            if (res.data.flag) {
            ...
            // 获取motor
            getType() {
                this.$axios.get('/pumpms/motor/query').then(res => {
                    res.data.forEach(v => {
                        this.types.push({
            ...
            getStards() {
                this.$axios.get('/pumpms/standard/query').then(res => {
                    res.data.forEach(v => {
                        this.standards.push({
                            key: v.stId,
                            label: v.stName
